...
const OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    thoughtFlow: {
      type: 'array',
      description: '하루의 흐름. 시작→전환→결론 3단계. 생각이 드러나면 그것을, 아니면 활동의 흐름을. 기록 3개 이하일 때만 빈 배열.',
      items: {
        type: 'object',
        properties: {
    ...
    energyWords: {
      type: 'object',
      description: '기록에 실제로 쓰인 단어 중 활력(up)·소모(down). 각 1~4개. 원문 그대로. 이모지·문장부호·숫자 제외.',
      properties: {
...
C. thoughtFlow — 하루의 흐름을 시작→전환→결론 3단계로 정리한다. 기록에 생각·판단·결심이 드러나면 그것을 따라가고, 활동 기록뿐이면 '무엇으로 시작해 → 무엇이 바뀌고 → 어디에 이르렀는지'를 사용자의 표현을 살려 적는다. 각 단계는 한 문장. 기록이 3개 이하일 때만 빈 배열.

D. loops — "A가 B로 이어졌다"가 기록에서 실제로 읽힐 때만 (시도→결과, 자각→행동, 욕구→자제). 없으면 빈 배열.

E. energyWords — 기록에 실제로 쓰인 한글·영문 단어 중 활력이 느껴지는 것(up)과 소모가 느껴지는 것(down)을 원문 그대로 뽑는다. up·down 각각 1~4개를 찾는다. 활력은 움직임·해냄·완료·좋았던 표현(운동, 완료, 준비, 회식, 좋다 등), 소모는 늘어짐·피로·하기 싫음(뒹굴, 누움, 힘들다, 피곤 등)이다. 단어는 기록의 어휘 그대로 쓰고, 이모지·문장부호·숫자는 제외한다. 정말 해당 단어가 하나도 없을 때만 빈 배열.
...
문체: 한국어, '~했어요' 체 (headline·narrative·thoughtFlow·loops). 말을 거는 듯 부드럽되 과장하지 않는다. 이모지·느낌표 금지. 사용자를 '당신'이라 부르지 않는다. 예외는 rabbit.reason 하나뿐 — 그것만 사용자 1인칭 '~했다' 체의 일기다.`;
