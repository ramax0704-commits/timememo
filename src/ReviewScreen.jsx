...
function WeekView({ week, habitKeywords, dayHeadlines, dayRabbits, onPickDay, onEditHabits, onViewed }) {
  ...
  // 상단 날짜를 지난주로 넘기면 그 날이 든 주가 나온다 (8/31). 어느 주인지는 배지에 날짜 범위로만 적어 준다.
  const first = week.days[0].date, last = week.days[week.days.length - 1].date;
  const isCurrentWeek = week.days.some(d => d.future) || week.days[6].key === format(new Date(), 'yyyy-MM-dd');
  const rangeLabel = `${format(first, 'M월 d일')}–${format(last, first.getMonth() === last.getMonth() ? 'd일' : 'M월 d일')}`;
  ...
  return (
    <>
      <section className="day-summary shape" aria-label="이번 주 할 말이 많았던 때">
        <header className="day-summary-head">
          <span className="day-summary-title">할 말이 많았던 때</span>
          <span className="day-summary-count">{isCurrentWeek ? '이번 주' : rangeLabel}</span>
        </header>
